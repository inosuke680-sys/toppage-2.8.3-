...
##############################################################################
# Umaten トップページプラグイン v2.10.7 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.7.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.7.sh
#   chmod +x /tmp/deploy-v2.10.7.sh
#   sudo /tmp/deploy-v2.10.7.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.7"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.7 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.7 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.7 をインストール中..."
...
echo ""
log_success "v2.10.7 の変更内容:"
echo "  【重要】検索ウィジェットURLにnonceを追加してセキュリティチェックエラーを修正"
echo "  "
echo "  【v2.10.6の問題】"
echo "    - XML Sitemap Generator for Google等のプラグインを有効化すると"
echo "    - 検索ウィジェットURL（/?umaten_category=10&umaten_tag=426&umaten_search=1）にアクセスした際"
echo "    - 「セキュリティチェックに失敗しました。ページを再読み込みして再試行してください。」というエラーが発生"
echo "    - 原因：URLにumaten_search_nonceパラメータが含まれていないため"
echo "  "
echo "  【v2.10.7の修正内容】"
echo "    1. class-url-rewrite.php（130-133行目）："
echo "       リダイレクトURLにnonceを追加"
echo "       "
echo "       修正前："
echo "         \$redirect_url = home_url('/?umaten_category=...');"
echo "       "
echo "       修正後："
echo "         \$nonce = wp_create_nonce('umaten_search');"
echo "         \$redirect_url = home_url('/?umaten_category=...&umaten_search_nonce=' . \$nonce);"
echo "    "
echo "    2. umaten-toppage.php（143-147行目）："
echo "       リンク書き換えURLにもnonceを追加"
echo "       "
echo "       修正前："
echo "         \$search_url = home_url('/?umaten_category=...');"
echo "       "
echo "       修正後："
echo "         \$nonce = wp_create_nonce('umaten_search');"
echo "         \$search_url = home_url('/?umaten_category=...&umaten_search_nonce=' . \$nonce);"
echo "  "
echo "  【修正の効果】"
echo "    - XML Sitemap Generator等のプラグインと共存可能に"
echo "    - セキュリティチェックエラーが発生しなくなる"
echo "    - WordPress標準のnonce検証に準拠"
echo "  "
echo "  【その他の修正（継続）】"
...
echo "     → 「セキュリティチェックに失敗しました」エラーが出ないことを確認"
echo "  2. 検索ウィジェットURLに直接アクセス"
echo "     例: https://umaten.jp/?umaten_category=10&umaten_tag=426&umaten_search=1"
echo "     → エラーが出ず、正常に検索結果が表示されることを確認"
