 ...
 * Description: 動的なカテゴリ・タグ表示を備えたトップページ用プラグイン。全エリア対応の3ステップナビゲーション（親→子カテゴリ→ジャンル）。SEO最適化・URLリライト完全修正（タグ・投稿判定改善）・ヒーロー画像メタデータ保存（SWELLテーマ完全対応）。検索結果ページ対応（モダンUI）。独自アクセスカウント機能搭載。投稿とタグの完全な区別。デバッグログ強化・エラーハンドリング改善。v2.10.0：【重要修正】投稿URLがホームページとして表示される問題を完全解決。umaten-restaurant-search-widgetのリライトルールが投稿URLを誤認識していた問題を修正し、投稿スラッグを検出して適切なWordPressクエリ変数に変換。
 * Version: 2.10.0
 * Author: Umaten
