...
**リダイレクトバグの根本的解決 + サイト説明セクションの視認性改善**

### 重大バグ修正

1. **カスタムリライトルールを追加してリダイレクトバグを根本的に解決**
   - **問題**: v2.8.7でも `/親/子/` URL（例: `https://umaten.jp/hokkaido/hakodate/`）がトップページにリダイレクトされる
   - **原因**: 404エラーの場合のみカスタム処理が実行されるため、WordPressが別の解釈をした場合に処理されない
   - **解決策**: WordPressのリライトルール機能を使用して、カスタムURLパターンを登録
     - `add_rewrite_rule()` で `/親/子/` と `/親/子/タグ/` パターンを登録
     - `query_vars` フィルターでカスタムクエリ変数を追加
     - `template_redirect` の優先度1で早期処理を実行
   - **技術詳細**:
     - `class-url-rewrite.php` に `add_rewrite_rules()` メソッドを追加
     - `add_query_vars()` メソッドで `umaten_parent`, `umaten_child`, `umaten_tag` を登録
     - `handle_custom_request()` メソッドで早期にリクエストをキャッチ
   - **影響**: 「すべてのジャンル」クリック時に確実にアーカイブページが表示される
   - **注意**: デプロイ後にパーマリンク設定の更新が必要（自動実行）
