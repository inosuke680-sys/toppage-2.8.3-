...
class Umaten_Toppage_Shortcode {
    ...
    public function render_shortcode($atts) {
        ...
        <div class="meshimap-wrapper">
            ...
            <!-- サイト説明 -->
            <section class="meshimap-section" id="about-section">
                <div class="meshimap-container">
                    <h2>&#127869; サイトについて</h2>
                    <div class="meshimap-about-content">
                        <?php echo $this->render_about_section(); ?>
                    </div>
                </div>
            </section>
            <?php endif; ?>
    ...
    private function get_monthly_access_count() {
        // 管理画面からの手動入力値を最優先
        $manual_access = get_option('umaten_toppage_monthly_access', '');
        if (!empty($manual_access) && is_numeric($manual_access)) {
            return number_format($manual_access) . '+';
        }
    ...
    private function render_about_section() {
        $output = '<div class="meshimap-about-description">';
        $output .= '<p>当サイトは、全国の美味しいお店を探せる日本最大級のグルメポータルサイトです。</p>';
        $output .= '<p>地域別・ジャンル別に飲食店を検索でき、実際の口コミや詳細情報をご覧いただけます。</p>';
        $output .= '<div class="meshimap-about-features">';
        $output .= '<div class="meshimap-feature-item">';
        $output .= '<div class="meshimap-feature-icon">&#128205;</div>';
        $output .= '<div class="meshimap-feature-text">';
        $output .= '<h3>全国対応</h3>';
        $output .= '<p>北海道から沖縄まで、全国各地のお店を掲載しています。</p>';
        $output .= '</div>';
        $output .= '</div>';
        $output .= '<div class="meshimap-feature-item">';
        $output .= '<div class="meshimap-feature-icon">&#11088;</div>';
        $output .= '<div class="meshimap-feature-text">';
        $output .= '<h3>信頼できる口コミ</h3>';
        $output .= '<p>実際に訪れた方々のリアルな口コミ情報が満載です。</p>';
        $output .= '</div>';
        $output .= '</div>';
        $output .= '<div class="meshimap-feature-item">';
        $output .= '<div class="meshimap-feature-icon">&#128269;</div>';
        $output .= '<div class="meshimap-feature-text">';
        $output .= '<h3>簡単検索</h3>';
        $output .= '<p>エリアやジャンルから、お探しのお店をすぐに見つけられます。</p>';
        $output .= '</div>';
        $output .= '</div>';
        $output .= '</div>';
        $output .= '</div>';
