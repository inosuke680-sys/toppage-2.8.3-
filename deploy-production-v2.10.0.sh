...
##############################################################################
# Umaten トップページプラグイン v2.10.0 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.0.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.0.sh
...
PLUGIN_NAME="umaten-toppage-v2.10.0"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.0 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.0 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.0 をインストール中..."
...
echo ""
log_success "v2.10.0 の変更内容:"
echo "  【最重要修正】投稿URLがホームページとして表示される問題を完全解決"
echo "  "
echo "  【問題の原因】"
echo "    - umaten-restaurant-search-widget の広範なリライトルール"
echo "      ^([^/]+)/([^/]+)/?$ が投稿URLを誤ってキャプチャ"
echo "    - /hokkaido/menya-kagetsu-hakodate-kikyo-2/ のような投稿URLが"
echo "      umaten_region=hokkaido, umaten_area=menya-kagetsu-hakodate-kikyo-2"
echo "      として誤認識される"
echo "    - WordPressは投稿を見つけられず、ホームページにフォールバック"
echo "    - is_404() も true にならないため、handle_404_redirect() も実行されない"
echo "  "
echo "  【v2.10.0の修正内容】"
echo "    handle_plugin_conflicts() メソッドに新しいロジックを追加："
echo "    1. umaten_area や umaten_genre が実際には投稿スラッグかチェック"
echo "    2. 投稿が見つかれば、カテゴリスラッグも検証"
echo "    3. 適切な WordPress クエリ変数（category_name + name）に変換"
echo "    4. widget のクエリ変数をクリア"
echo "  "
echo "  【期待される動作】"
echo "    1. URL: /hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "    2. Widget rule matches: umaten_region=hokkaido, umaten_area=menya-kagetsu-hakodate-kikyo-2"
echo "    3. handle_plugin_conflicts() detects 'menya-kagetsu-hakodate-kikyo-2' is a post slug"
echo "    4. Converts to: category_name=hokkaido, name=menya-kagetsu-hakodate-kikyo-2"
echo "    5. WordPress correctly loads the post"
echo "    6. Post displays with correct title and content"
echo "  "
echo "  【その他の利点】"
echo "    - restaurant-review-category-tagsが無効でも投稿ページが正常に動作"
echo "    - umaten-restaurant-search-widgetとの完全な共存"
echo "    - カテゴリ+タグページも引き続き正常に動作"
echo ""
log_warning "次のステップ:"
