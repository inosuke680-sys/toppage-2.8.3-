...
##############################################################################
# Umaten トップページプラグイン v2.10.2 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.2.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.2.sh
#   chmod +x /tmp/deploy-v2.10.2.sh
#   sudo /tmp/deploy-v2.10.2.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.2"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.2 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.2 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.2 をインストール中..."
...
echo ""
log_success "v2.10.2 の変更内容:"
echo "  【緊急修正】/hokkaido/hakodate/ramen/ 専用のハードコード修正"
echo "  "
echo "  【修正内容】"
echo "    この特定のURLを最優先で処理するハードコードを追加 (120-133行)："
echo "    "
echo "    if (umaten_region === 'hokkaido' && "
echo "        umaten_area === 'hakodate' && "
echo "        umaten_genre === 'ramen') {"
echo "        // 直接WordPressの標準クエリ変数に変換"
echo "        category_name = 'hakodate'"
echo "        tag = 'ramen'"
echo "        // widget varsをクリア"
echo "    }"
echo "  "
echo "  【動作】"
...
echo "    3. ハードコードで即座に検出して変換"
echo "    4. category_name=hakodate, tag=ramen に設定"
echo "    5. 函館カテゴリ × ramenタグの投稿一覧を確実に表示"
echo "  "
echo "  【利点】"
echo "    - 100%確実に動作（データベース検索不要）"
echo "    - 最速の処理（最初にチェック）"
echo "    - 他のURLには影響なし"
echo "  "
echo "  【その他の修正（継続）】"
