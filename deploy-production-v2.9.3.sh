...
##############################################################################
# Umaten トップページプラグイン v2.9.3 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.3.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.3.sh
#   chmod +x /tmp/deploy-v2.9.3.sh
#   sudo /tmp/deploy-v2.9.3.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.3"
OLD_PLUGIN_PATTERN="umaten-toppage-v2.9.*"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.3 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.3 をインストール中..."
...
echo ""
log_success "v2.9.3 の変更内容:"
echo "  1. v2.9.2の過剰な条件判定を修正"
echo "     - is_archive()条件が広すぎて検索ウィジェットのカスタムアーカイブページまでクリアされる問題を修正"
echo "     - WordPress標準のクエリ変数が実際に設定されている場合のみクリアするように変更"
echo "  2. 検索ウィジェットのカスタムアーカイブページが正常に動作"
echo "     - /hokkaido/niseko/ramen/ → ニセコのラーメンが正しく表示される"
echo "     - 誤って函館のラーメンが表示される問題を解決"
echo "  3. 記事クリック時の挙動が正常化"
echo "     - トップページやカテゴリページに誤って遷移する問題を解決"
echo "  4. 両プラグインが完全に共存可能"
echo "     - 投稿ページ、検索ウィジェットのアーカイブページ、WordPress標準のアーカイブページすべてが正常動作"
echo ""
log_warning "次のステップ:"
echo "  1. 検索ウィジェットのカスタムアーカイブページをテスト"
echo "     - 例: /hokkaido/niseko/ramen/ が正しくニセコのラーメンを表示することを確認"
echo "  2. 投稿ページが正常に表示されることを確認"
echo "  3. 記事をクリックして正常に遷移することを確認"
