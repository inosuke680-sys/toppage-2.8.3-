...
##############################################################################
# Umaten トップページプラグイン v2.9.2 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.2.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.2.sh
#   chmod +x /tmp/deploy-v2.9.2.sh
#   sudo /tmp/deploy-v2.9.2.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.2"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.2 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.2 をインストール中..."
...
echo ""
log_success "v2.9.2 の変更内容:"
echo "  1. umaten-restaurant-search-widgetとの衝突を回避"
echo "     - 検索ウィジェットプラグインの広範囲なリライトルールと共存可能に"
echo "     - WordPress標準のクエリを優先し、両プラグインの機能を維持"
echo "  2. ジャンルタグクリック時のリダイレクト問題を解決"
echo "     - エリア選択 → 子カテゴリ選択 → ジャンル選択が正常に動作"
echo "     - 投稿ページが正常に表示される"
echo "  3. 検索ウィジェットの機能も正常に動作"
echo "     - umaten-restaurant-search-widgetの検索機能に影響なし"
echo ""
log_warning "次のステップ:"
echo "  1. サイトにアクセスして、エリア選択 → 子カテゴリ選択 → ジャンル選択 をテスト"
echo "  2. ジャンルタグをクリックしてもトップページにリダイレクトされないことを確認"
