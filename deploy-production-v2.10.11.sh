...
##############################################################################
# Umaten トップページプラグイン v2.10.11 本番環境デプロイスクリプト（確実な解決版）
#
# v2.10.11 新機能：
# - モーダルで検索URLを直接生成（rewrite rulesに依存しない）
# - カテゴリID + タグID で検索ウィジェットURLを構築
# - 確実にすべてのジャンル（izakaya、cafe、italian等）に対応
# - OPcache完全クリアで確実に新コードを実行
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.11.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.11.sh
#   chmod +x /tmp/deploy-v2.10.11.sh
#   sudo /tmp/deploy-v2.10.11.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.11"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.11 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.11 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.11 をインストール中..."
...
echo ""
log_success "v2.10.11 の変更内容:"
echo "  【重要】検索URLを直接生成する方式に変更（確実な解決策）"
echo "  "
echo "  【問題の背景】"
echo "    - v2.10.9/v2.10.10はrewrite rulesに依存していた"
echo "    - しかしrewrite rulesが/hokkaido/hakodate/izakayaをキャッチしていなかった"
echo "    - parse_queryフックは呼ばれるが、クエリ変数が設定されない"
echo "    - そのため、タグ検出ロジックが実行されなかった"
echo "  "
echo "  【v2.10.11の解決方法】"
echo "    1. モーダルで直接検索URLを生成"
echo "    2. URLパターン: ?umaten_category=10&umaten_tag=432&umaten_search=1&umaten_search_nonce=..."
echo "    3. rewrite rulesに依存せず、直接検索ウィジェットに渡す"
echo "    4. カテゴリID + タグID を使って確実に検索実行"
echo "  "
echo "  【技術的な変更】"
echo "    - toppage.js: 子カテゴリIDを保存（data-child-id属性）"
echo "    - toppage.js: タグクリック時に検索URLを生成（/region/area/tag ではなく ?umaten_category=X&umaten_tag=Y）"
echo "    - class-shortcode.php: searchNonceをJavaScriptに渡す"
echo "  "
echo "  【動作例】"
echo "    修正前: https://umaten.jp/hokkaido/hakodate/izakaya （動作しない）"
echo "    修正後: https://umaten.jp/?umaten_category=10&umaten_tag=432&umaten_search=1&... （確実に動作）"
echo "  "
echo "  【メリット】"
echo "    - rewrite rulesに依存しない"
echo "    - parse_queryフックに依存しない"
echo "    - 検索ウィジェットが直接URLを処理"
echo "    - すべてのジャンル（izakaya、cafe、italian等）で確実に動作"
echo ""
log_warning "次のステップ:"
echo "  1. トップページの地域選択を開く"
echo "     https://umaten.jp/"
echo "  "
echo "  2. モーダルで「北海道 > 函館 > 居酒屋・バー」を選択"
echo "     → ブラウザの開発者ツール（F12）でネットワークタブを確認"
echo "     → リンクが ?umaten_category=10&umaten_tag=432&... の形式になっているはず"
echo "  "
echo "  3. クリックして検索ウィジェットページに遷移することを確認"
echo "     → 居酒屋・バーの検索結果が表示されるはず"
echo "  "
echo "  4. 他のジャンルもテスト（cafe、italian、ラーメン等）"
echo "     → すべて検索ウィジェットに正しく遷移するはず"
echo "  "
echo "  5. ブラウザのキャッシュもクリア（Ctrl+F5 または Cmd+Shift+R）"
echo "  "
echo "  6. JavaScriptコンソールでログを確認"
echo "     → \"[v2.10.11] 検索URL生成\" というログが表示されるはず"
echo "     → \"カテゴリID:\" と \"タグID:\" が表示されるはず"
echo "  "
