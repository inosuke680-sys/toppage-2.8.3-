...
##############################################################################
# Umaten トップページプラグイン v2.10.9 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.9.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.9.sh
#   chmod +x /tmp/deploy-v2.10.9.sh
#   sudo /tmp/deploy-v2.10.9.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.9"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.9 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.9 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.9 をインストール中..."
...
echo ""
log_success "v2.10.9 の変更内容:"
echo "  【重要】すべてのカテゴリ+タグURLを自動検出して検索ウィジェットにリダイレクト"
echo "  "
echo "  【v2.10.8の問題】"
echo "    - ramen のみハードコード処理 → 正常動作"
echo "    - izakaya、cafe などの他のタグ → 投稿URLと誤認識される問題"
echo "    - 例: /hokkaido/hakodate/izakaya → /hokkaido/izakaya-michinoku-hakodate-matsukaze/ にリダイレクト"
echo "    - 原因: タグチェックより投稿チェックが先に実行されるため"
echo "  "
echo "  【v2.10.9の修正内容】"
echo "    3セグメントURL処理の順序を変更（class-url-rewrite.php）："
echo "    "
echo "    修正前の順序："
echo "      1. hokkaido/hakodate/ramen → ハードコード処理（リダイレクト）"
echo "      2. その他の3セグメントURL → 投稿チェック → タグチェック"
echo "    "
echo "    修正後の順序："
echo "      1. すべての3セグメントURL → タグチェック（優先）→ リダイレクト"
echo "      2. タグでない場合のみ → 投稿チェック"
echo "    "
echo "    実装（class-url-rewrite.php: 144-166行目）："
echo "      \$tag = get_term_by('slug', \$umaten_genre, 'post_tag');"
echo "      if (\$tag && \$child_cat) {"
echo "          \$nonce = wp_create_nonce('umaten_search_action');"
echo "          \$redirect_url = ...&umaten_search_nonce=' . \$nonce;"
echo "          wp_redirect(\$redirect_url, 301);"
echo "      }"
echo "  "
echo "  【動作例】"
echo "    /hokkaido/hakodate/ramen   → 検索ウィジェットURL（ramenタグ）"
echo "    /hokkaido/hakodate/izakaya → 検索ウィジェットURL（izakayaタグ）"
echo "    /hokkaido/hakodate/cafe    → 検索ウィジェットURL（cafeタグ）"
echo "    /hokkaido/hakodate/italian → 検索ウィジェットURL（italianタグ）"
echo "    "
echo "    すべてのカテゴリ+タグの組み合わせが自動的に検索ウィジェットにリダイレクト"
echo "  "
echo "  【修正の効果】"
echo "    - すべてのジャンル（タグ）に対応 - ハードコード不要"
echo "    - 投稿URLとの誤認識を防止"
echo "    - トップページのジャンル選択が全ジャンルで正常動作"
echo "    - メンテナンス性の大幅な向上"
echo "  "
echo "  【その他の修正（継続）】"
echo "    - v2.10.8: 正しいnonce名でnonce検証に対応"
echo "    - v2.10.6: トレーリングスラッシュなしURLを生成"
echo "    - v2.10.5: トレーリングスラッシュあり・なし両方に対応"
echo ""
log_warning "次のステップ:"
echo "  1. トップページの地域選択から様々なジャンルを試す："
echo "     - 北海道 > 函館 > ラーメン → 検索結果が表示されることを確認"
echo "     - 北海道 > 函館 > 居酒屋・バー → 検索結果が表示されることを確認"
echo "     - 北海道 > 函館 > カフェ・スイーツ → 検索結果が表示されることを確認"
echo "     - 北海道 > 函館 > イタリアン・フレンチ → 検索結果が表示されることを確認"
echo "  2. 各ジャンルのURLに直接アクセスして動作確認："
echo "     - https://umaten.jp/hokkaido/hakodate/izakaya"
echo "     - https://umaten.jp/hokkaido/hakodate/cafe"
echo "     - https://umaten.jp/hokkaido/hakodate/italian"
echo "     → すべて検索結果に正しくリダイレクトされることを確認"
echo "  3. 投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "     → 投稿URLと誤認識されていないことを確認"
echo "  4. カテゴリページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/"
echo "  5. ブラウザのキャッシュをクリア（Ctrl+F5 または Cmd+Shift+R）"
echo "  6. 問題がある場合は、バックアップから復元: $BACKUP_DIR"
echo ""
