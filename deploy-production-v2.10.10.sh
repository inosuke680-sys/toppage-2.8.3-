...
##############################################################################
# Umaten トップページプラグイン v2.10.10 本番環境デプロイスクリプト（診断強化版）
#
# v2.10.10 新機能：
# - parse_queryフック呼び出しを確認する詳細診断ログ
# - タグ検出ロジック（izakaya、cafeなど）の詳細ログ
# - カテゴリ検出の詳細ログ
# - すべてのカテゴリ+タグURLの自動リダイレクト対応
# - OPcache完全クリアで確実にv2.10.10コードを実行
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.10.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.10.sh
#   chmod +x /tmp/deploy-v2.10.10.sh
#   sudo /tmp/deploy-v2.10.10.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.10"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.10 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.10 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.10 をインストール中..."
...
echo ""
log_success "v2.10.10 の変更内容:"
echo "  【重要】診断ログを大幅強化し、v2.10.9が実行されない問題を特定"
echo "  "
echo "  【問題の背景】"
echo "    - v2.10.9は/hokkaido/hakodate/izakaya等の全ジャンルに対応するはずだった"
echo "    - しかし実際にはv2.8.3のコードが実行され、izakayaが投稿URLにリダイレクトされていた"
echo "    - 原因：OPcacheによる古いバイトコードのキャッシュ、またはプラグインの初期化問題"
echo "  "
echo "  【v2.10.10の診断機能】"
echo "    1. parse_queryフックが呼ばれているかを確認（バージョン番号を出力）"
echo "    2. 検索ウィジェットのクエリ変数（region/area/genre）の検出をログ出力"
echo "    3. タグ検出の詳細ログ（'izakaya'がタグとして見つかるか）"
echo "    4. カテゴリ検出の詳細ログ（'hakodate'がカテゴリとして見つかるか）"
echo "    5. リダイレクト実行時の詳細ログ（どのURLにリダイレクトするか）"
echo "  "
echo "  【診断ログの例】"
echo "    Umaten Toppage v2.10.10: parse_query hook called - Version: 2.10.10"
echo "    Umaten Toppage v2.10.10: Search widget vars detected - region=hokkaido, area=hakodate, genre=izakaya"
echo "    Umaten Toppage v2.10.10: Processing 3-segment URL - region=hokkaido, area=hakodate, genre=izakaya"
echo "    Umaten Toppage v2.10.10: Tag check for 'izakaya' - FOUND (ID: 123, Name: 居酒屋・バー)"
echo "    Umaten Toppage v2.10.10: Category check for 'hakodate' - FOUND (ID: 10, Name: 函館)"
echo "    Umaten Toppage v2.10.10: REDIRECTING /hokkaido/hakodate/izakaya/ to search widget URL"
echo "  "
echo "  【OPcacheクリア強化】"
echo "    - OPcacheリセット実行"
echo "    - PHP-FPM再起動でOPcacheを完全クリア"
echo "    - 確実にv2.10.10のコードが実行される"
echo "  "
echo "  【継続機能（v2.10.9から）】"
echo "    - すべてのカテゴリ+タグURLを自動検出（izakaya、cafe、italian等）"
echo "    - タグ優先チェック（投稿より先にタグをチェック）"
echo "    - 検索ウィジェットURLへの自動リダイレクト"
echo ""
log_warning "次のステップ:"
echo "  1. デバッグログを確認してv2.10.10が実行されていることを確認"
echo "     tail -f /home/kusanagi/45515055731ac663c7c3ad4c/DocumentRoot/wp-content/debug.log"
echo "  "
echo "  2. 問題のあったizakayaリンクをテスト"
echo "     https://umaten.jp/hokkaido/hakodate/izakaya にアクセス"
echo "     → ログに \"Umaten Toppage v2.10.10: REDIRECTING\" が表示されるはず"
echo "     → 検索ウィジェットURLに正しくリダイレクトされるはず"
echo "  "
echo "  3. 他のジャンルもテスト（cafe、italian等）"
echo "     https://umaten.jp/hokkaido/hakodate/cafe"
echo "     → 同様に検索ウィジェットURLにリダイレクトされるはず"
echo "  "
echo "  4. ラーメンも引き続き動作することを確認"
echo "     https://umaten.jp/hokkaido/hakodate/ramen"
echo "  "
echo "  5. 投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "  "
echo "  6. ブラウザのキャッシュもクリア（Ctrl+F5 または Cmd+Shift+R）"
echo "  "
echo "  7. ログに \"v2.8.3\" や他の古いバージョン番号が表示される場合："
echo "     - OPcacheが完全にクリアされていない可能性"
echo "     - サーバー再起動を検討: sudo systemctl restart nginx php-fpm"
echo "  "
