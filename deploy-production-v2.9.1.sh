...
##############################################################################
# Umaten トップページプラグイン v2.9.1 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.1.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.1.sh
#   chmod +x /tmp/deploy-v2.9.1.sh
#   sudo /tmp/deploy-v2.9.1.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.1"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.1 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.1 をインストール中..."
...
echo ""
log_success "v2.9.1 の変更内容:"
echo "  1. カスタムリライトルールを削除（投稿ページ表示バグ修正）"
echo "     - v2.8.8で追加したカスタムリライトルールが投稿ページURLに誤マッチ"
echo "     - 投稿ページが表示されない問題を解決"
echo "  2. 削除したメソッド："
echo "     - add_rewrite_rules() メソッド"
echo "     - add_query_vars() メソッド"
echo "     - handle_custom_request() メソッド"
echo "  3. 404ハンドラーのみ残して通常動作に戻す"
echo ""
log_warning "次のステップ:"
echo "  1. 投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "  2. ジャンルアーカイブページが正常に動作することを確認"
echo "     エリア選択 → 子カテゴリ選択 → ジャンル選択"
echo "  3. ブラウザのキャッシュもクリア（Ctrl+F5 または Cmd+Shift+R）"
