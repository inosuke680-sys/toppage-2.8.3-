...
##############################################################################
# Umaten トップページプラグイン v2.9.9 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.9.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.9.sh
#   chmod +x /tmp/deploy-v2.9.9.sh
#   sudo /tmp/deploy-v2.9.9.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.9"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.9 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.9 をインストール中..."
...
echo ""
log_success "v2.9.9 の変更内容:"
echo "  【重要】handle_404_redirect() を再度有効化し、rrct_active チェックを追加"
echo "  "
echo "  【v2.9.8の問題】"
echo "    handle_404_redirect() を無効化したことで、以下の問題が発生："
echo "    "
echo "    1. 投稿ページが正しく表示されない（すべての投稿リスト表示）"
echo "    2. カテゴリ+タグページがトップページにリダイレクト"
echo "    "
echo "    理由："
echo "    handle_404_redirect() は単なるリダイレクトではなく、"
echo "    404を検出して適切なページ（投稿/アーカイブ）として表示する"
echo "    重要な機能でした。これを無効化すると、投稿ページやアーカイブ"
echo "    ページが正しく表示されなくなります。"
echo "  "
echo "  【v2.9.9の修正】"
echo "    handle_404_redirect() を再度有効化し、メソッドの最初に"
echo "    rrct_active チェックを追加："
echo "    "
echo "    if (get_query_var('rrct_active')) {"
echo "        return; // restaurant-review-category-tags が処理中"
echo "    }"
echo "    "
echo "    これにより："
echo "    - 投稿ページ: setup_single_post_query() が正常に動作 ✓"
echo "    - カテゴリページ: setup_archive_query() が正常に動作 ✓"
echo "    - カテゴリ+タグページ: rrct_active がある場合はスキップ ✓"
echo "    - restaurant-review-category-tags が優先的に処理 ✓"
echo "  "
echo "  【期待される動作】"
echo "    1. 投稿ページ: 単一記事として正しく表示される ✓"
echo "    2. カテゴリページ: カテゴリアーカイブとして表示される ✓"
echo "    3. カテゴリ+タグページ: restaurant-review-category-tags が処理 ✓"
echo "    4. トップページ: 正常に表示される ✓"
echo ""
log_warning "次のステップ:"
echo "  1. サイトにアクセスして、カテゴリ+タグページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/ramen/"
echo "  2. 投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "  3. ジャンルアーカイブページが正常に動作することを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/ramen/"
