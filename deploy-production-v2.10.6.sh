...
##############################################################################
# Umaten トップページプラグイン v2.10.6 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.6.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.6.sh
#   chmod +x /tmp/deploy-v2.10.6.sh
#   sudo /tmp/deploy-v2.10.6.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.6"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.6 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.6 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.6 をインストール中..."
...
echo ""
log_success "v2.10.6 の変更内容:"
echo "  【重要】ジャンル選択モーダルのURLをトレーリングスラッシュなしに修正"
echo "  "
echo "  【v2.10.5の問題】"
echo "    - v2.10.5では出力バッファでリンク書き換え（トレーリングスラッシュあり・なし両方対応）"
echo "    - しかし、ジャンル選択モーダルのJavaScriptがトレーリングスラッシュありのURLを生成"
echo "    - 結果：モーダル内のリンクは検索URLに書き換えられるが、非効率"
echo "  "
echo "  【v2.10.6の修正内容】"
echo "    toppage.js（368行目）を修正してトレーリングスラッシュなしのURLを生成："
echo "    "
echo "    修正前："
echo "      const tagUrl = umatenToppage.siteUrl + '/' + ... + tag.slug + '/';"
echo "                                                                    ^^^ スラッシュあり"
echo "    "
echo "    修正後："
echo "      const tagUrl = umatenToppage.siteUrl + '/' + ... + tag.slug;"
...
echo "  "
echo "  【動作】"
echo "    ジャンル選択モーダルで生成されるURL："
echo "    修正前: https://umaten.jp/hokkaido/hakodate/ramen/"
echo "    修正後: https://umaten.jp/hokkaido/hakodate/ramen"
echo "    "
echo "    これにより、v2.10.4のリンク書き換えロジックと整合性が確保されます。"
echo "  "
echo "  【修正の効果】"
echo "    - ジャンル選択モーダルのリンクが直接正しい形式で生成される"
echo "    - v2.10.4のリンク書き換えとの整合性が向上"
echo "    - より効率的でシンプルな実装"
echo "  "
echo "  【その他の修正（継続）】"
...
echo ""
log_warning "次のステップ:"
echo "  1. トップページの地域選択から「北海道 > 函館 > ラーメン」をクリック"
echo "     → 検索ウィジェットURLに正しく遷移することを確認"
echo "  2. ジャンル選択モーダル内のリンクが正しい形式になっていることを確認"
echo "     （ブラウザの開発者ツールでリンクのhref属性を確認）"
