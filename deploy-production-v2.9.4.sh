...
##############################################################################
# Umaten トップページプラグイン v2.9.4 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.4.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.4.sh
#   chmod +x /tmp/deploy-v2.9.4.sh
#   sudo /tmp/deploy-v2.9.4.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.4"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.4 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.4 をインストール中..."
...
echo ""
log_success "v2.9.4 の変更内容:"
echo "  1. 複数プラグインとの衝突を完全に回避"
echo "     - umaten-restaurant-search-widget との共存が可能"
echo "     - restaurant-review-category-tags との共存が可能"
echo "     - WordPress標準のクエリを優先し、すべてのプラグイン機能を維持"
echo "  2. 投稿ページとアーカイブページが正常に動作"
echo "     - 投稿ページ（例: /hokkaido/menya-kagetsu-hakodate-kikyo-2/）が正しく表示"
echo "     - ジャンルアーカイブページが正しく表示"
echo "     - カテゴリ+タグページが正しく表示"
echo "  3. すべてのプラグイン機能が共存可能"
echo "     - umaten-toppage のトップページ機能"
echo "     - umaten-restaurant-search-widget の検索機能"
echo "     - restaurant-review-category-tags のカテゴリ+タグ機能"
echo ""
log_warning "次のステップ:"
echo "  1. サイトにアクセスして、投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "  2. ジャンルアーカイブページが正常に動作することを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/ramen/"
echo "  3. 検索ウィジェットの機能が正常に動作することを確認"
echo "  4. restaurant-review-category-tagsの機能が正常に動作することを確認"
