...
##############################################################################
# Umaten トップページプラグイン v2.10.1 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.1.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.1.sh
#   chmod +x /tmp/deploy-v2.10.1.sh
#   sudo /tmp/deploy-v2.10.1.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.1"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.1 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.1 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.1 をインストール中..."
...
echo ""
log_success "v2.10.1 の変更内容:"
echo "  【最終修正】カテゴリ+タグURL (/hokkaido/hakodate/ramen/) の問題を完全解決"
echo "  "
echo "  【問題の原因】"
echo "    - v2.10.0では umaten_genre が投稿スラッグの場合のみ処理していた"
echo "    - /hokkaido/hakodate/ramen/ のようなカテゴリ+タグURLで："
echo "      1. umaten_genre='ramen' はタグだが、投稿スラッグとしてチェックされる"
echo "      2. 投稿が見つからないため、何も処理されず widget vars が残る"
echo "      3. umaten-restaurant-search-widgetまたはフォールバック処理が"
echo "         umaten_area='hakodate' のみを使用してカテゴリページを表示"
echo "      4. 結果：タグ 'ramen' が無視される"
echo "  "
echo "  【v2.10.1の修正内容】"
echo "    handle_plugin_conflicts() メソッドにタグ検出ロジックを追加 (182-216行)："
echo "    1. umaten_genre が投稿スラッグかチェック（既存）"
echo "    2. 【新規】投稿でない場合、タグ (post_tag) かチェック"
echo "    3. タグが見つかれば、umaten_region/umaten_area がカテゴリか検証"
echo "    4. 適切な WordPress クエリ変数に変換："
echo "       - category_name: hakodate (または hokkaido/hakodate)"
echo "       - tag: ramen"
echo "    5. widget のクエリ変数をクリア"
echo "  "
echo "  【期待される動作】"
echo "    1. URL: /hokkaido/hakodate/ramen/"
echo "    2. Widget rule matches: umaten_region=hokkaido, umaten_area=hakodate, umaten_genre=ramen"
echo "    3. handle_plugin_conflicts() detects 'ramen' is a tag (not a post)"
echo "    4. Converts to: category_name=hakodate, tag=ramen"
echo "    5. WordPress correctly displays category+tag archive"
echo "    6. Page shows '函館のラーメン' posts"
echo "  "
echo "  【その他の修正】"
echo "    - v2.10.0: 投稿URLの誤認識問題を解決（継続）"
echo "    - restaurant-review-category-tagsが無効でもカテゴリ+タグページが動作"
echo "    - umaten-restaurant-search-widgetとの完全な共存"
echo ""
log_warning "次のステップ:"
echo "  1. 投稿ページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/menya-kagetsu-hakodate-kikyo-2/"
echo "     → 投稿の正しいタイトルと内容が表示されるはず"
echo "  2. カテゴリページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/"
echo "  3. カテゴリ+タグページが正常に表示されることを確認"
echo "     例: https://umaten.jp/hokkaido/hakodate/ramen/"
echo "     （restaurant-review-category-tagsが有効な場合）"
echo "  4. 検索ウィジェットの機能が正常に動作することを確認"
echo "  5. ブラウザのキャッシュもクリア（Ctrl+F5 または Cmd+Shift+R）"
