...
##############################################################################
# Umaten トップページプラグイン v2.9.7 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.7.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.7.sh
#   chmod +x /tmp/deploy-v2.9.7.sh
#   sudo /tmp/deploy-v2.9.7.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.7"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.7 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.7 をインストール中..."
...
echo ""
log_success "v2.9.7 の変更内容:"
echo "  【重要】template_redirect → parse_query フックに変更"
echo "  "
echo "  【v2.9.6の問題】"
echo "    template_redirect の優先度を15に変更しましたが、"
echo "    これにより投稿ページまでトップページにリダイレクトされる問題が発生。"
echo "    優先度のタイミング調整だけでは根本解決にならないことが判明。"
echo "  "
echo "  【v2.9.7の根本的修正】"
echo "    1. template_redirect から parse_query フックに変更（優先度999）"
echo "    2. WP_Query 解析直後に実行され、すべてのプラグインの処理後に確実に実行"
echo "    3. \$query オブジェクトを直接操作することで、より確実な判定が可能"
echo "    4. is_main_query() チェックにより、メインクエリのみを処理"
echo "  "
echo "  【期待される動作】"
echo "    1. WordPress が URL をパースし、WP_Query を解析"
echo "    2. すべてのプラグインが parse_query で処理を実行"
echo "    3. umaten-toppage が優先度999で最後に実行され、rrct_active をチェック"
echo "    4. rrct_active が true なら何もせず return → カテゴリ+タグページ表示"
echo "    5. rrct_active が false かつ投稿ページなら競合 query vars をクリア → 投稿ページ表示"
echo "  "
echo "  【技術的改善点】"
echo "    - get_query_var() → \$query->get() に変更（直接WP_Queryから取得）"
echo "    - set_query_var() → \$query->set() に変更（WP_Queryに直接設定）"
echo "    - is_main_query() チェックにより、サブクエリには影響しない"
echo ""
log_warning "次のステップ:"
