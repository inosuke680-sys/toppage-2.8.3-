...
##############################################################################
# Umaten トップページプラグイン v2.8.6 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.8.6.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.8.6.sh
#   chmod +x /tmp/deploy-v2.8.6.sh
#   sudo /tmp/deploy-v2.8.6.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.8.6"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.8.6 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.8.6 をインストール中..."
...
echo ""
log_success "新機能:"
log_success "v2.8.6の新機能:"
echo "  1. プレミアムデザイン実装（予算80万円レベル）"
echo "     - グラスモーフィズム統計バー・検索バー"
echo "     - 複雑なグラデーションアニメーション"
echo "     - テキストグラデーションクリップ効果"
echo "     - 多重シャドウ・発光エフェクト"
echo "  2. ジャンル「すべて」選択時のバグ修正（トップページリダイレクト解消）"
