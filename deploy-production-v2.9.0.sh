...
##############################################################################
# Umaten トップページプラグイン v2.9.0 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.0.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.0.sh
...
PLUGIN_NAME="umaten-toppage-v2.9.0"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.0 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.0 をインストール中..."
...
echo ""
log_success "v2.9.0 の変更内容:"
echo "  1. 「すべてのジャンル」ボタンを削除"
echo "     - リダイレクトバグの暫定対応として機能を削除"
echo "     - ユーザーは個別のジャンル（タグ）を選択する必要がある"
echo "  2. 各ジャンルボタンは正常に機能"
echo "     - エリア選択 → 子カテゴリ選択 → ジャンル選択"
echo ""
log_warning "次のステップ:"
echo "  1. サイトにアクセスして、エリア選択 → 子カテゴリ選択 → ジャンル選択 をテスト"
echo "  2. 「すべてのジャンル」ボタンが表示されないことを確認"
echo "  3. 各ジャンルボタンが正常に動作することを確認"
