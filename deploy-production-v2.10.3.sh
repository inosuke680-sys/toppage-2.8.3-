...
##############################################################################
# Umaten トップページプラグイン v2.10.3 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.10.3.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.10.3.sh
#   chmod +x /tmp/deploy-v2.10.3.sh
#   sudo /tmp/deploy-v2.10.3.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.10.3"
...
echo ""
echo "=========================================="
echo "  Umaten プラグイン v2.10.3 デプロイ"
echo "=========================================="
echo ""
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.10.3 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.10.3 をインストール中..."
...
echo ""
log_success "v2.10.3 の変更内容:"
echo "  /hokkaido/hakodate/ramen/ を検索ウィジェットURLにリダイレクト"
echo "  "
echo "  【修正内容】"
echo "    /hokkaido/hakodate/ramen/ を検索ウィジェットの結果ページにリダイレクト (120-138行)："
echo "    "
echo "    if (umaten_region === 'hokkaido' && "
echo "        umaten_area === 'hakodate' && "
echo "        umaten_genre === 'ramen') {"
echo "        // カテゴリとタグのIDを動的に取得"
echo "        hakodate_cat_id = get_term_by('slug', 'hakodate', 'category')->term_id"
echo "        ramen_tag_id = get_term_by('slug', 'ramen', 'post_tag')->term_id"
echo "        "
echo "        // 検索ウィジェットURLに301リダイレクト"
echo "        wp_redirect('/?umaten_category={cat_id}&umaten_tag={tag_id}&umaten_search=1', 301)"
echo "    }"
echo "  "
echo "  【動作】"
echo "    1. URL: /hokkaido/hakodate/ramen/ にアクセス"
echo "    2. Widget vars: umaten_region=hokkaido, umaten_area=hakodate, umaten_genre=ramen"
echo "    3. 函館カテゴリとramenタグのIDを動的に取得"
echo "    4. 検索ウィジェットURL: /?umaten_category=10&umaten_tag=426&umaten_search=1 にリダイレクト"
echo "    5. 検索ウィジェットが函館のラーメン店を表示"
echo "  "
echo "  【利点】"
echo "    - 検索ウィジェット機能を活用（既存の動作している機能）"
echo "    - SEO対策：301リダイレクトで検索エンジンに正しいURLを通知"
echo "    - IDを動的に取得するため、データベース変更に強い"
echo "  "
echo "  【その他の修正（継続）】"
echo "    - v2.10.0: 投稿URLの誤認識問題を解決"
echo "    - v2.10.1: 一般的なカテゴリ+タグURLのタグ検出ロジック"
echo ""
log_warning "次のステップ:"
