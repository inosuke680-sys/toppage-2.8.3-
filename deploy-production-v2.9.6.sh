...
##############################################################################
# Umaten トップページプラグイン v2.9.6 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.6.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.6.sh
#   chmod +x /tmp/deploy-v2.9.6.sh
#   sudo /tmp/deploy-v2.9.6.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.6"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.6 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.6 をインストール中..."
...
echo ""
log_success "v2.9.6 の変更内容:"
echo "  【重要】template_redirect フックの優先度を 1 → 15 に変更"
echo "  "
echo "  【問題の原因】"
echo "    v2.9.5では、umaten-toppage が優先度1で実行され、"
echo "    restaurant-review-category-tags が rrct_active を設定する前に"
echo "    チェックが走り、誤って rrct_* 変数をクリアしていました。"
echo "  "
echo "  【v2.9.6の修正】"
echo "    優先度を15に変更することで、restaurant-review-category-tags が"
echo "    先に query vars を設定する時間を確保。これによりタイミング問題を完全解決。"
echo "  "
echo "  【期待される動作】"
echo "    1. WordPress が URL をパースし、rewrite rules にマッチ"
echo "    2. restaurant-review-category-tags が rrct_active などを設定（優先度10）"
echo "    3. umaten-toppage が rrct_active をチェック（優先度15）"
echo "    4. rrct_active が true なので何もせず return"
echo "    5. カテゴリ+タグページが正常に表示される"
echo ""
log_warning "次のステップ:"
