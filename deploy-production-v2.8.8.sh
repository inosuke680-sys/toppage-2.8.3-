...
##############################################################################
# Umaten トップページプラグイン v2.8.8 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.8.8.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.8.8.sh
#   chmod +x /tmp/deploy-v2.8.8.sh
#   sudo /tmp/deploy-v2.8.8.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.8.8"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.8.8 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.8.8 をインストール中..."
...
# 10. パーマリンクの更新（v2.8.8で必須）
if [ "$WP_CLI_AVAILABLE" = true ]; then
    log_info "パーマリンクを更新中（リライトルール適用のため必須）..."
    cd "$DOCROOT"
    wp rewrite flush --path="$DOCROOT"
    log_success "パーマリンク更新完了 - カスタムリライトルールが有効化されました"
else
    log_warning "【重要】WordPress管理画面 → 設定 → パーマリンク → 「変更を保存」を必ず実行してください。"
fi
...
echo "=========================================="
echo ""
log_info "バージョン: v2.8.8"
log_info "インストール先: $INSTALL_PATH"
log_info "バックアップ: $BACKUP_DIR"
echo ""
log_success "実行された処理:"
echo "  ✓ プラグインのインストールと有効化"
echo "  ✓ パーマリンクの更新（カスタムリライトルール適用）"
echo "  ✓ 全キャッシュのクリア（OPcache、WordPress、KUSANAGI、Nginx、PHP-FPM）"
echo ""
log_success "v2.8.8 の新機能:"
echo "  1. リダイレクトバグを根本的に解決"
echo "     - カスタムリライトルールを追加（/親/子/ と /親/子/タグ/ パターン）"
echo "     - template_redirect優先度1で早期処理"
echo "     - 「すべてのジャンル」が確実にアーカイブページを表示"
echo "  2. サイト説明セクションの視認性を大幅改善"
echo "     - 背景の透明度を調整してテキストを読みやすく"
echo "     - アニメーションを控えめに調整"
echo "     - ヒーローセクションとの統一感を強化"
echo ""
log_warning "次のステップ:"
echo "  1. サイトにアクセスして、エリア選択 → 子カテゴリ選択 → すべてのジャンル をテスト"
echo "  2. アーカイブページが正しく表示されることを確認"
echo "  3. サイト説明セクションの視認性を確認"
