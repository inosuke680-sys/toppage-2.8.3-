...
##############################################################################
# Umaten トップページプラグイン v2.9.8 本番環境デプロイスクリプト
#
# 実行方法:
#   curl -o /tmp/deploy-v2.9.8.sh https://raw.githubusercontent.com/inosuke680-sys/toppage-2.8.3-/claude/remove-popular-genre-update-layout-01Q3CQHiR3WZZMyJCDjpyMhq/deploy-production-v2.9.8.sh
#   chmod +x /tmp/deploy-v2.9.8.sh
#   sudo /tmp/deploy-v2.9.8.sh
##############################################################################
...
PLUGIN_NAME="umaten-toppage-v2.9.8"
...
# 4. 新しいプラグインのダウンロード
log_info "新しいプラグイン v2.9.8 をダウンロード中..."
mkdir -p "$TEMP_DIR"
cd "$TEMP_DIR"
...
# 7. 新しいプラグインのインストール
log_info "新しいプラグイン v2.9.8 をインストール中..."
...
echo ""
log_success "v2.9.8 の変更内容:"
echo "  【重要】handle_404_redirect() メソッドを完全無効化"
echo "  "
echo "  【v2.9.7の問題 - デバッグログから判明した真の原因】"
echo "    包括的デバッグログの分析により、以下が判明："
echo "    "
echo "    1. /hokkaido/hakodate/ → 404として検出"
echo "    2. /hokkaido/hakodate/ramen/ → 404として検出"
echo "    3. handle_404_redirect() が template_redirect 優先度999で実行"
echo "    4. 404を検出してトップページまたはカテゴリページにリダイレクト"
echo "    "
echo "    つまり、parse_query の変更は無関係で、"
echo "    元々存在していた handle_404_redirect() が真の犯人でした。"
echo "  "
echo "  【v2.9.8の根本的修正】"
echo "    handle_404_redirect() メソッドの呼び出しを完全にコメントアウト："
echo "    // add_action('template_redirect', array(\$this, 'handle_404_redirect'), 999);"
echo "    "
echo "    これにより："
echo "    - 404ページは WordPress 標準の404テンプレートで処理される"
echo "    - 他のプラグインのリライトルールが正しく機能する"
echo "    - restaurant-review-category-tags が自由に動作できる"
echo "  "
echo "  【期待される動作】"
echo "    1. 投稿ページ: 正常に表示される ✓"
echo "    2. カテゴリページ: 正常に表示される ✓"
echo "    3. カテゴリ+タグページ: restaurant-review-category-tags が処理 ✓"
echo "    4. トップページ: 正常に表示される ✓"
echo "    5. 真の404ページ: WordPress 標準の404テンプレート表示 ✓"
echo ""
log_warning "次のステップ:"
